...
export const fetchDashboardData = async () => {
    ...
    try {
      ...
      `)
        ...
        // Note: The Flutter code references 'pro_inventory!inner(name)', assuming 'pro_inventory' is the relation used.
        // If 'pro_inventory' doesn't exist, we might need to check the actual relation name.
        // However, following the Flutter code strictly.
        ...
        const { data: targetData, error: targetError } = await supabase
            ...
            .eq('start_date', todayStr) // Assuming targets are daily or have a start_date matches today
            .maybeSingle(); // Use maybeSingle to avoid 406 if no target

        const productionTarget = targetData?.target_quantity || 120; // Default 120 from Flutter
        ...
        let totalMachines = 15; // Default from Flutter if table empty?

        if (!machinesError && machinesData) {
            totalMachines = machinesData.length || 15; // Keep default if 0?
            activeMachines = machinesData.filter(m => m.status === 'running').length;
        }

        // 6. Quality Rate (Mocked like Flutter for now as no explicit table shown)
        // Flutter: 95.0 + random
